...
Namespace Presentadores.FacReportes
    Class PresentadorPendientesRpt
        Inherits PresentadorBase

        Private _ventana As IPendientesRpt
        ...
        'Function poner_decimal(ByVal valor As String)
        '    Dim retorna As String
        '    Dim posicion As Integer            
        '    retorna = Replace(valor, ",", ".")
        '    posicion = InStr(retorna, ".")
        '    If posicion < 1 Then
        '        retorna = retorna & ".00"
        '    End If
        '    Return retorna
        ...
        Private Function GetRutaReporte() As String
            ...
            'retorno = Environment.CurrentDirectory & ConfigurationManager.AppSettings("rutaFacreportes") & "RptFacturaPendienteDeta2.rpt"
        ...
        Public Function Buscar_Operacion(ByVal nc As String, ByVal asociado3 As Asociado, ByVal tipo As Integer) As List(Of FacOperacion)
            ...
            '    valor = True
            'Else
            If Me._ventana.Fecha2 IsNot Nothing And Me._ventana.Fecha2.ToString <> "" Then
                operacionaux.ValorQuery = "Select o from FacOperacion o left join fetch o.Asociado as Asociado left join fetch o.Moneda as Moneda left join fetch o.Idioma as Idioma"
                operacionaux.ValorQuery = operacionaux.ValorQuery & " where o.FechaOperacion <='" & Me._ventana.Fecha2 & "'"
                valor = True
            End If
            'End If
            If tipo = 1 Then
            ...
            If tipo = 2 Then
                If asociado3 IsNot Nothing Then
                    If asociado3.Id > Integer.MinValue Then
                        'Dim asociado As Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                        If valor = False Then
                            operacionaux.ValorQuery = "Select o from FacOperacion o left join fetch o.Asociado as Asociado left join fetch o.Moneda as Moneda left join fetch o.Idioma as Idioma"
                            operacionaux.ValorQuery = operacionaux.ValorQuery & " where "
                        Else
                            operacionaux.ValorQuery = operacionaux.ValorQuery & " and "
                        End If
                        operacionaux.ValorQuery = operacionaux.ValorQuery & " Asociado.Id= " & asociado3.Id
                        valor = True
                    End If
                End If
            End If
            ...
            ' Para obtener las FacOperaciones tomando en cuenta el combo de Condicion de Dias de Credito
            If tipo = 1 Then
        ...
        Public Function Buscar_OperacionPais(ByVal nc As String, ByVal asociado3 As Asociado, ByVal tipo As Integer) As List(Of FacOperacionPais)
        ...
        Public Function sumar_abono(ByVal asociado As Asociado, ByVal tipo As Integer) As Double
            Dim monto As Double = 0
            If tipo = 1 Then
                Dim facOperacion As List(Of FacOperacion) = Buscar_Operacion("NC", asociado, 2)
                ...
                Return (monto)
            Else
                Dim facOperacion As List(Of FacOperacionPais) = Buscar_OperacionPais("NC", asociado, 2)
                For i As Integer = 0 To facOperacion.Count - 1
                    If (facOperacion(i).Moneda.Id.Equals("US")) Then
                        monto = monto + (facOperacion(i).Saldo * -1)
                    Else
        ...
        Public Function ObtenerEstructuraEncPais(ByVal operacion As FacOperacionPais) As StructReporteFActuraEnc
            Dim structura As New StructReporteFActuraEnc()
            Dim w_s As String = ""
            structura = inicializar_enc()
            Try
        ...
        Private Sub ObtenerEstructura(ByRef enc As IList(Of StructReporteFActuraEnc), ByRef det As List(Of StructReporteFActuraDeta))
            ...
            Try
                ...
                If seleccionpais = False Then
                    Dim facOperacion As List(Of FacOperacion) = Buscar_Operacion("ND", Nothing, 1)
                    For i As Integer = 0 To facOperacion.Count - 1
                        If i = 0 Then
                            'llamar encabezado
                    ...
                    '''''''''''''''''''''''''''''''''''''
                Else 'si es pais
                    Dim facOperacionPais As List(Of FacOperacionPais) = Buscar_OperacionPais("ND", Nothing, 1)
                    For i As Integer = 0 To facOperacionPais.Count - 1
                        ...
                        Else
                            ...
                            End If
                        End If
                        ObtenerEstructuraDetaPais(j, det, facOperacionPais(i), montototal)
        ...
        Private Sub ObtenerEstructuraDetaPais(ByVal id As String, ByVal detalle As List(Of StructReporteFActuraDeta), ByVal operacion As FacOperacionPais, ByRef montototal As Double)
            If consultar_cobro_factura(operacion.CodigoOperacion) = False Then
                Dim retorno As IList(Of StructReporteFActuraDeta) = New List(Of StructReporteFActuraDeta)
                retorno = detalle
                Dim monto As Double = 0
        ...
        Private Function ArmarReporteEnc(ByVal datos As DataTable, ByVal estructurasDeDatos As IList(Of StructReporteFActuraEnc)) As DataTable
            Try
                For Each structura As StructReporteFActuraEnc In estructurasDeDatos
                    ...
                    filaDatos("Mabono") = poner_decimal(structura.Mabono)
