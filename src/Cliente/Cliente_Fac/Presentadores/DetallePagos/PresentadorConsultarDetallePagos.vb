...
Namespace Presentadores.DetallePagos
    Class PresentadorConsultarDetallePagos
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarDetallePagos
        Private _DetallePagoServicios As IDetallePagoServicios
        Private _DetallePagos As IList(Of DetallePago)

        ''' <summary>
        ''' Constructor Predeterminado
        ''' </summary>
        ''' <param name="ventana">página que satisface el contrato</param>
        Public Sub New(ByVal ventana As IConsultarDetallePagos)
            Try
                Me._ventana = ventana
        ...
        Public Sub ActualizarTitulo()
            Me.ActualizarTituloVentanaPrincipal(Recursos.Etiquetas.fac_titleConsultarDetallePagos, Recursos.Ids.fac_ConsultarDetallePago)
        End Sub
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._DetallePagos = Me._DetallePagoServicios.ConsultarTodos()
                Me._ventana.Count = Me._DetallePagos.Count
                Me._ventana.Resultados = Me._DetallePagos
                Me._ventana.DetallePagoFiltrar = New DetallePago()
        ...
        Public Sub Limpiar()
            ...
            Me.Navegar(New ConsultarDetallePagos())
        ...
        Public Sub Consultar()
            Try
                ...
                Dim DetallePago As DetallePago = DirectCast(Me._ventana.DetallePagoFiltrar, DetallePago)
                'DetallePago.Region = If(Not Me._ventana.Region.Equals(""), Me._ventana.Region, Nothing)
                'DetallePago.Id = Nothing
                Dim DetallePagosFiltrados As IEnumerable(Of DetallePago) = Me._DetallePagos


                If Not String.IsNullOrEmpty(DetallePago.Id) Then
                    DetallePagosFiltrados = From p In DetallePagosFiltrados Where p.Id IsNot Nothing AndAlso p.Id.ToLower().Contains(DetallePago.Id.ToLower())
                End If

                If Not String.IsNullOrEmpty(DetallePago.Descripcion) Then
                    DetallePagosFiltrados = From p In DetallePagosFiltrados Where p.Descripcion IsNot Nothing AndAlso p.Descripcion.ToLower().Contains(DetallePago.Descripcion.ToLower())
                End If
        ...
        ''' <summary>
        ''' Método que invoca una nueva página "ConsultarDetallePago" y la instancia con el objeto seleccionado
        ''' </summary>
        Public Sub IrConsultarDetallePago()
            '#Region "trace"
            If ConfigurationManager.AppSettings("ambiente").ToString().Equals("desarrollo") Then
                logger.Debug("Entrando al metodo {0}", (New System.Diagnostics.StackFrame()).GetMethod().Name)
            End If
            ...
            Me.Navegar(New ConsultarDetallePago(Me._ventana.DetallePagoSeleccionado))
            'Me.Navegar(New ConsultarDetallePago())
            '#Region "trace"
            If ConfigurationManager.AppSettings("ambiente").ToString().Equals("desarrollo") Then
                logger.Debug("Saliendo del metodo {0}", (New System.Diagnostics.StackFrame()).GetMethod().Name)
            End If
