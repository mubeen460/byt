...
Namespace Presentadores.FacCobros
    Class PresentadorConsultarFacCobros
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacCobros
        ...
        Public Sub New(ByVal ventana As IConsultarFacCobros)
            Try
                Me._ventana = ventana
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._ventana.Resultados = Me._FacCobros
                Me._ventana.FacCobroFiltrar = New FacCobro()
        ...
        Public Sub Limpiar()
            ...
            Me.Navegar(New ConsultarFacCobros())
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacCobroAuxiliar As New FacCobro()
                Dim FacCobros As FacCobro = DirectCast(_ventana.FacCobroFiltrar, FacCobro)

                If Not Me._ventana.Id.Equals("") Then
                    FacCobroAuxiliar.Id = Integer.Parse(Me._ventana.Id)
                End If

                'If Not Me._ventana.CreditoSent.Equals("") Then
                '    FacCobroAuxiliar.CreditoSent = Integer.Parse(Me._ventana.CreditoSent)
                'End If

                If (Me._ventana.Asociado IsNot Nothing) AndAlso (DirectCast(Me._ventana.Asociado, Asociado).Id <> Integer.MinValue) Then
                    FacCobroAuxiliar.Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                End If

                If (Me._ventana.Banco IsNot Nothing) AndAlso (DirectCast(Me._ventana.Banco, FacBanco).Id <> Integer.MinValue) Then
                    FacCobroAuxiliar.Banco = DirectCast(Me._ventana.Banco, FacBanco)
                End If

                If (Me._ventana.Idioma IsNot Nothing) AndAlso (DirectCast(Me._ventana.Idioma, Idioma).Id <> "") Then
                    FacCobroAuxiliar.Idioma = DirectCast(Me._ventana.Idioma, Idioma)
                End If

                If (Me._ventana.Moneda IsNot Nothing) AndAlso (DirectCast(Me._ventana.Moneda, Moneda).Id <> "") Then
                    FacCobroAuxiliar.Moneda = DirectCast(Me._ventana.Moneda, Moneda)
                End If

                If Not Me._ventana.FechaCobro.Equals("") Then
                    Dim FechaFacCobro As DateTime = DateTime.Parse(Me._ventana.FechaCobro)
                    FacCobroAuxiliar.FechaCobro = FechaFacCobro
                End If

                'If (filtroValido = True) Then
                Me._FacCobros = Me._FacCobroServicios.ObtenerFacCobrosFiltro(FacCobroAuxiliar)
                Me._ventana.Count = Me._FacCobros.Count
                If Me._FacCobros.Count <= 0 Then
