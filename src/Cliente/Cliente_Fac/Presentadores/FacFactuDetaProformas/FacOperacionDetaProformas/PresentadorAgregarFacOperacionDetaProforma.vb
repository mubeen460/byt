...
Namespace Presentadores.FacOperacionDetaProformas
    Class PresentadorAgregarFacOperacionDetaProforma
        Inherits PresentadorBase
        Private _ventana As IAgregarFacOperacionDetaProforma
        ...
        Private _FacOperacionDetaProformaFacturaServicios As IFacOperacionDetaProformaFacturaServicios
        Dim xoperacion As String
        ''' <summary>
        ''' Constructor predeterminado
        ''' </summary>
        ''' <param name="ventana">Página que satisface el contrato</param>
        Public Sub New(ByVal ventana As IAgregarFacOperacionDetaProforma)
            Try
                Me._ventana = ventana
                'Me._ventana.FacOperacionDetaProforma = New FacOperacionDetaProforma()
                ...
                Me._FacOperacionDetaProformaFacturaServicios = DirectCast(Activator.GetObject(GetType(IFacOperacionDetaProformaFacturaServicios), ConfigurationManager.AppSettings("RutaServidor") + ConfigurationManager.AppSettings("FacOperacionDetaProformaFacturaServicios")), IFacOperacionDetaProformaFacturaServicios)
        ...
        Public Sub CargarPagina()
            ...
            Try
                Me.ActualizarTituloVentanaPrincipal(Recursos.Etiquetas.fac_titleAgregarFacOperacionDetaProforma, Recursos.Ids.AgregarUsuario)
                Me._ventana.MensajeErrorCobro = ""

                Dim FacOperacionDetaProforma As New FacOperacionDetaProforma()
                FacOperacionDetaProforma.FechaCobro = Date.Now
                Me._ventana.FacOperacionDetaProforma = FacOperacionDetaProforma
        ...
        Public Sub Aceptar()
            Try
                Dim FacOperacionDetaProforma As FacOperacionDetaProforma = DirectCast(_ventana.FacOperacionDetaProforma, FacOperacionDetaProforma)

                FacOperacionDetaProforma.Asociado = If(Not DirectCast(Me._ventana.Asociado, Asociado).Id.Equals("NGN"), DirectCast(Me._ventana.Asociado, Asociado), Nothing)

                'If Not Me._FacCreditoServicios.VerificarExistencia(FacCredito) Then                
                FacOperacionDetaProforma.Moneda = If(Not DirectCast(Me._ventana.Moneda, Moneda).Id.Equals("NGN"), DirectCast(Me._ventana.Moneda, Moneda), Nothing)
                FacOperacionDetaProforma.Idioma = If(Not DirectCast(Me._ventana.Idioma, Idioma).Id.Equals("NGN"), DirectCast(Me._ventana.Idioma, Idioma), Nothing)
                FacOperacionDetaProforma.Banco = If(Not DirectCast(Me._ventana.Banco, FacBanco).Id.Equals("NGN"), DirectCast(Me._ventana.Banco, FacBanco), Nothing)

                Dim valido As Boolean = True
                If (FacOperacionDetaProforma.Moneda.Id = "US") Then
                ...
                If (valido = True) Then
                    ...
                    FacOperacionDetaProforma.Id = contador.ProximoValor
                    ...
                    Dim FacOperacionDetaProformaFactura As List(Of FacOperacionDetaProformaFactura) = DirectCast(Me._ventana.ResultadosFacturaCobro, List(Of FacOperacionDetaProformaFactura))
                    ModificarFacOperacionesCobroxFactura(FacOperacionDetaProformaFactura)
                    Dim exitoso As Boolean = _FacOperacionDetaProformaServicios.InsertarOModificar(FacOperacionDetaProforma, UsuarioLogeado.Hash)

                    If exitoso Then
                        AgregarFacFormaCobro(FacFormas, FacOperacionDetaProforma)
                        AgregarFacOperacionDetaProformaFacturaCobro(FacOperacionDetaProformaFactura, FacOperacionDetaProforma)

                        Dim operacion As New FacOperacion
                        operacion.Id = "NP"
                        operacion.CodigoOperacion = FacOperacionDetaProforma.Id
                        operacion.FechaOperacion = FacOperacionDetaProforma.FechaCobro
                        operacion.Asociado = FacOperacionDetaProforma.Asociado
                        operacion.Idioma = FacOperacionDetaProforma.Idioma
                        operacion.Moneda = FacOperacionDetaProforma.Moneda
                        ...
                        Me.Navegar(Recursos.MensajesConElUsuario.fac_FacOperacionDetaProformaInsertado, False)
                    End If
                    'Else
                    'Me._ventana.Mensaje(Recursos.MensajesConElUsuario.fac_ErrorFacOperacionDetaProformaRepetido)
                    ' End If
                End If
            Catch ex As ApplicationException
                logger.[Error](ex.Message)
        ...
        Public Sub AgregarFacFormaCobro(ByVal FacFormas As List(Of FacForma), ByVal Cobro As FacOperacionDetaProforma)
            For i As Integer = 0 To FacFormas.Count - 1
        ...
        Public Sub ModificarFacOperacionesCobroxFactura(ByVal CobroxFacturas As List(Of FacOperacionDetaProformaFactura))
        ...
        Public Sub AgregarFacOperacionDetaProformaFacturaCobro(ByVal FacOperacionDetaProformaFacturas As List(Of FacOperacionDetaProformaFactura), ByVal Cobro As FacOperacionDetaProforma)
            For i As Integer = 0 To FacOperacionDetaProformaFacturas.Count - 1
                FacOperacionDetaProformaFacturas.Item(i).Id = Cobro
                Dim exitoso As Boolean = Me._FacOperacionDetaProformaFacturaServicios.InsertarOModificar(FacOperacionDetaProformaFacturas.Item(i), UsuarioLogeado.Hash)
            Next
        End Sub
        ...
        Public Sub AgregarFacturas2()
            ...
            Dim FacOperacionDetaProformaFactura As New List(Of FacOperacionDetaProformaFactura)
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    FacOperacionDetaProformaFactura.Add(New FacOperacionDetaProformaFactura)
                    Dim cobro As New FacOperacionDetaProforma
                    cobro.Id = 0
                    FacOperacionDetaProformaFactura.Item(j).Id = cobro
                    FacOperacionDetaProformaFactura.Item(j).Factura = FacOperacion.Item(i).CodigoOperacion
                    FacOperacionDetaProformaFactura.Item(j).Bono = FacOperacion.Item(i).Saldo
                    FacOperacionDetaProformaFactura.Item(j).BonoBf = FacOperacion.Item(i).SaldoBf
                    If (Moneda.Id = "US") Then
                        sumabono = sumabono + FacOperacionDetaProformaFactura.Item(j).Bono
                    Else
                        If (Moneda.Id = "BS") Then
                            sumabono = sumabono + FacOperacionDetaProformaFactura.Item(j).Bono / 2.15
                        Else
                            If (Moneda.Id = "BF") Then
                                sumabono = sumabono + FacOperacionDetaProformaFactura.Item(j).BonoBf
                            End If
                        End If

                    End If
                    sumabonobf = sumabonobf + FacOperacionDetaProformaFactura.Item(j).BonoBf

                    If (xoperacion = "") Then
                        xoperacion = FacOperacionDetaProformaFactura.Item(j).Factura
                    Else
                        xoperacion = xoperacion & "," & FacOperacionDetaProformaFactura.Item(j).Factura
                    End If

                    j = j + 1
                End If
            Next
            Me._ventana.ResultadosFacturaCobro = FacOperacionDetaProformaFactura
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    ...
                    If (FacOperacion.Item(i).Moneda.Id = "US") Then
                        ...
                        tasa.Id = FacOperacionDetaProforma.FechaCobro
