...
namespace Trascend.Bolet.Cliente.Presentadores.EscritosPatente
{
    class PresentadorContestacionAOposicion : PresentadorBase
    {
        ...
        public string GenerarString()
        {
            ...
            if(null!=this._ventana.Oponente)
                StringLlleno += this._ventana.Oponente + "  ";
            ...
            try
            {
                ...
                if (ValidarEscrito())
                {
                    ...
                    this.EjecutarArchivoBAT(ConfigurationManager.AppSettings["RutaBatEscrito"].ToString()
                        + "\\" + ConfigurationManager.AppSettings["EscritoContestacionAOposicion"].ToString(),
                        ((Agente)this._ventana.AgenteFiltrado).Id + " " + ((Resolucion)this._ventana.Resolucion).Id + " " +
                        ((Boletin)this._ventana.Boletin).Id + " " + this._ventana.Oponente + " " +
                        this._ventana.Domiciliado + " " + DateTime.Parse(this._ventana.Fecha).ToShortDateString()
                        + " " + parametroPatentes);
                }
