...
namespace Trascend.Bolet.AccesoDatos.Recursos {
    ...
    public class ConsultasHQL {
        ...
        /// <summary>
        ///   Looks up a localized string similar to Select p from Patente p left join fetch p.Asociado as asociado left join fetch p.Interesado as interesado left join fetch asociado.Pais as pais left join fetch asociado.Idioma as pais left join fetch p.BoletinConcesion as boletinC left join fetch p.BoletinPublicacion as boletinP left join fetch p.BoletinOrdenPublicacion as boletinO left join fetch p.Anualidades as anualidades left join fetch p.Agente as agente left join fetch p.TipoEstado as tipoEstado left join fetch p.StatusWeb as statusWeb where .
        /// </summary>
        ...
        /// <summary>
        ///   Looks up a localized string similar to Select f from Fecha where f.Id = {0}.
        ///   Busca una cadena traducida similar a Select f from Fecha f left outer join fetch f.Tipo as tipoFecha left outer join fetch f.Correspondencia as carta left outer join fetch carta.Resumen as resumen where f.Id = {0}.
        /// </summary>
        ...
        /// <summary>
        ///   Looks up a localized string similar to Select p from Patente p left join fetch p.Asociado as asociado left join fetch p.Servicio as servicio left join fetch p.Inventores as inventores left join fetch p.Memoria as memoria where p.Id = &apos;{0}&apos; order by p.Id.
        /// </summary>
