...
Namespace Presentadores.FacOperacionDetalleTms
    Class PresentadorConsultarFacOperacionDetalleTms
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacOperacionDetalleTms
        ...
        Public Sub New(ByVal ventana As IConsultarFacOperacionDetalleTms)
            Try
                Me._ventana = ventana
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._ventana.Resultados = Me._FacOperacionDetalleTms
                Me._ventana.FacOperacionDetalleTmFiltrar = New FacOperacionDetalleTm()
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacOperacionDetalleTmAuxiliar As New FacOperacionDetalleTm()
                Dim FacOperacionDetalleTms As FacOperacionDetalleTm = DirectCast(_ventana.FacOperacionDetalleTmFiltrar, FacOperacionDetalleTm)

                If Not Me._ventana.Id.Equals("") Then
                    FacOperacionDetalleTmAuxiliar.Id = Integer.Parse(Me._ventana.Id)
                End If

                'If Not Me._ventana.CreditoSent.Equals("") Then
                '    FacOperacionDetalleTmAuxiliar.CreditoSent = Integer.Parse(Me._ventana.CreditoSent)
                'End If

                If (Me._ventana.Asociado IsNot Nothing) AndAlso (DirectCast(Me._ventana.Asociado, Asociado).Id <> Integer.MinValue) Then
                    FacOperacionDetalleTmAuxiliar.Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                End If

                If (Me._ventana.Banco IsNot Nothing) AndAlso (DirectCast(Me._ventana.Banco, FacBanco).Id <> Integer.MinValue) Then
                    FacOperacionDetalleTmAuxiliar.Banco = DirectCast(Me._ventana.Banco, FacBanco)
                End If

                If (Me._ventana.Idioma IsNot Nothing) AndAlso (DirectCast(Me._ventana.Idioma, Idioma).Id <> "") Then
                    FacOperacionDetalleTmAuxiliar.Idioma = DirectCast(Me._ventana.Idioma, Idioma)
                End If

                If (Me._ventana.Moneda IsNot Nothing) AndAlso (DirectCast(Me._ventana.Moneda, Moneda).Id <> "") Then
                    FacOperacionDetalleTmAuxiliar.Moneda = DirectCast(Me._ventana.Moneda, Moneda)
                End If

                If Not Me._ventana.FechaCobro.Equals("") Then
                    Dim FechaFacOperacionDetalleTm As DateTime = DateTime.Parse(Me._ventana.FechaCobro)
                    FacOperacionDetalleTmAuxiliar.FechaCobro = FechaFacOperacionDetalleTm
                End If
