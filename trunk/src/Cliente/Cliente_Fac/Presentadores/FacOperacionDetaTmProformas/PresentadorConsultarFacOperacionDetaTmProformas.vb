...
Namespace Presentadores.FacOperacionDetaTmProformas
    Class PresentadorConsultarFacOperacionDetaTmProformas
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacOperacionDetaTmProformas
        ...
        Public Sub New(ByVal ventana As IConsultarFacOperacionDetaTmProformas)
            Try
                Me._ventana = ventana
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._ventana.Resultados = Me._FacOperacionDetaTmProformas
                Me._ventana.FacOperacionDetaTmProformaFiltrar = New FacOperacionDetaTmProforma()
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacOperacionDetaTmProformaAuxiliar As New FacOperacionDetaTmProforma()
                Dim FacOperacionDetaTmProformas As FacOperacionDetaTmProforma = DirectCast(_ventana.FacOperacionDetaTmProformaFiltrar, FacOperacionDetaTmProforma)

                If Not Me._ventana.Id.Equals("") Then
                    FacOperacionDetaTmProformaAuxiliar.Id = Integer.Parse(Me._ventana.Id)
                End If

                'If Not Me._ventana.CreditoSent.Equals("") Then
                '    FacOperacionDetaTmProformaAuxiliar.CreditoSent = Integer.Parse(Me._ventana.CreditoSent)
                'End If

                If (Me._ventana.Asociado IsNot Nothing) AndAlso (DirectCast(Me._ventana.Asociado, Asociado).Id <> Integer.MinValue) Then
                    FacOperacionDetaTmProformaAuxiliar.Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                End If

                If (Me._ventana.Banco IsNot Nothing) AndAlso (DirectCast(Me._ventana.Banco, FacBanco).Id <> Integer.MinValue) Then
                    FacOperacionDetaTmProformaAuxiliar.Banco = DirectCast(Me._ventana.Banco, FacBanco)
                End If

                If (Me._ventana.Idioma IsNot Nothing) AndAlso (DirectCast(Me._ventana.Idioma, Idioma).Id <> "") Then
                    FacOperacionDetaTmProformaAuxiliar.Idioma = DirectCast(Me._ventana.Idioma, Idioma)
                End If

                If (Me._ventana.Moneda IsNot Nothing) AndAlso (DirectCast(Me._ventana.Moneda, Moneda).Id <> "") Then
                    FacOperacionDetaTmProformaAuxiliar.Moneda = DirectCast(Me._ventana.Moneda, Moneda)
                End If

                If Not Me._ventana.FechaCobro.Equals("") Then
                    Dim FechaFacOperacionDetaTmProforma As DateTime = DateTime.Parse(Me._ventana.FechaCobro)
                    FacOperacionDetaTmProformaAuxiliar.FechaCobro = FechaFacOperacionDetaTmProforma
                End If
