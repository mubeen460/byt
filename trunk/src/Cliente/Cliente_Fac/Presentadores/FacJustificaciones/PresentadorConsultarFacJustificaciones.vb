...
Namespace Presentadores.FacJustificaciones
    Class PresentadorConsultarFacJustificaciones
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacJustificaciones
        Private _FacJustificacionServicios As IFacJustificacionServicios
        Private _FacJustificaciones As IList(Of FacJustificacion)

        ''' <summary>
        ''' Constructor Predeterminado
        ''' </summary>
        ''' <param name="ventana">página que satisface el contrato</param>
        Public Sub New(ByVal ventana As IConsultarFacJustificaciones)
            Try
                Me._ventana = ventana
        ...
        Public Sub Limpiar()
            ...
            Me.Navegar(New ConsultarFacJustificaciones())
        ...
        Public Sub ActualizarTitulo()
            Me.ActualizarTituloVentanaPrincipal(Recursos.Etiquetas.fac_titleConsultarFacJustificaciones, Recursos.Ids.fac_ConsultarFacJustificacion)
        End Sub
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._FacJustificaciones = Me._FacJustificacionServicios.ConsultarTodos()
                Me._ventana.Count = Me._FacJustificaciones.Count
                Me._ventana.Resultados = Me._FacJustificaciones
                Me._ventana.FacJustificacionFiltrar = New FacJustificacion()
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacJustificacion As FacJustificacion = DirectCast(Me._ventana.FacJustificacionFiltrar, FacJustificacion)
                'FacJustificacion.Region = If(Not Me._ventana.Region.Equals(""), Me._ventana.Region, Nothing)
                'FacJustificacion.Id = Nothing
                Dim FacJustificacionsFiltrados As IEnumerable(Of FacJustificacion) = Me._FacJustificaciones


                If Not String.IsNullOrEmpty(FacJustificacion.Id) Then
                    FacJustificacionsFiltrados = From p In FacJustificacionsFiltrados Where p.Id IsNot Nothing AndAlso p.Id.ToLower().Contains(FacJustificacion.Id.ToLower())
                End If

                If Not String.IsNullOrEmpty(FacJustificacion.Concepto) Then
                    FacJustificacionsFiltrados = From p In FacJustificacionsFiltrados Where p.Concepto IsNot Nothing AndAlso p.Concepto.ToLower().Contains(FacJustificacion.Concepto.ToLower())
                End If
        ...
        ''' <summary>
        ''' Método que invoca una nueva página "ConsultarFacJustificacion" y la instancia con el objeto seleccionado
        ''' </summary>
        Public Sub IrConsultarFacJustificacion()
            '#Region "trace"
            If ConfigurationManager.AppSettings("ambiente").ToString().Equals("desarrollo") Then
                logger.Debug("Entrando al metodo {0}", (New System.Diagnostics.StackFrame()).GetMethod().Name)
            End If
            ...
            Me.Navegar(New ConsultarFacJustificacion(Me._ventana.FacJustificacionSeleccionado))
            'Me.Navegar(New ConsultarFacJustificacion())
            '#Region "trace"
            If ConfigurationManager.AppSettings("ambiente").ToString().Equals("desarrollo") Then
                logger.Debug("Saliendo del metodo {0}", (New System.Diagnostics.StackFrame()).GetMethod().Name)
            End If
