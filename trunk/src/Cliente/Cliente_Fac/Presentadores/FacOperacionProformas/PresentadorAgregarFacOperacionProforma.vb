...
Namespace Presentadores.FacOperacionProformas
    Class PresentadorAgregarFacOperacionProforma
        Inherits PresentadorBase
        Private _ventana As IAgregarFacOperacionProforma
        ...
        Private _FacOperacionProformaFacturaServicios As IFacOperacionProformaFacturaServicios
        Dim xoperacion As String
        ''' <summary>
        ''' Constructor predeterminado
        ''' </summary>
        ''' <param name="ventana">Página que satisface el contrato</param>
        Public Sub New(ByVal ventana As IAgregarFacOperacionProforma)
            Try
                Me._ventana = ventana
                'Me._ventana.FacOperacionProforma = New FacOperacionProforma()
                ...
                Me._FacOperacionProformaFacturaServicios = DirectCast(Activator.GetObject(GetType(IFacOperacionProformaFacturaServicios), ConfigurationManager.AppSettings("RutaServidor") + ConfigurationManager.AppSettings("FacOperacionProformaFacturaServicios")), IFacOperacionProformaFacturaServicios)

                Dim FacOperacionProforma As New FacOperacionProforma()
        ...
        Public Sub CargarPagina()
            ...
            Try
                Me.ActualizarTituloVentanaPrincipal(Recursos.Etiquetas.fac_titleAgregarFacOperacionProforma, Recursos.Ids.AgregarUsuario)
                Me._ventana.MensajeErrorCobro = ""

                Dim FacOperacionProforma As New FacOperacionProforma()
                FacOperacionProforma.FechaCobro = Date.Now
                Me._ventana.FacOperacionProforma = FacOperacionProforma
        ...
        Public Sub Aceptar()
            Try
                Dim FacOperacionProforma As FacOperacionProforma = DirectCast(_ventana.FacOperacionProforma, FacOperacionProforma)

                FacOperacionProforma.Asociado = If(Not DirectCast(Me._ventana.Asociado, Asociado).Id.Equals("NGN"), DirectCast(Me._ventana.Asociado, Asociado), Nothing)

                'If Not Me._FacCreditoServicios.VerificarExistencia(FacCredito) Then                
                FacOperacionProforma.Moneda = If(Not DirectCast(Me._ventana.Moneda, Moneda).Id.Equals("NGN"), DirectCast(Me._ventana.Moneda, Moneda), Nothing)
                FacOperacionProforma.Idioma = If(Not DirectCast(Me._ventana.Idioma, Idioma).Id.Equals("NGN"), DirectCast(Me._ventana.Idioma, Idioma), Nothing)
                FacOperacionProforma.Banco = If(Not DirectCast(Me._ventana.Banco, FacBanco).Id.Equals("NGN"), DirectCast(Me._ventana.Banco, FacBanco), Nothing)

                Dim valido As Boolean = True
                If (FacOperacionProforma.Moneda.Id = "US") Then
                ...
                If (valido = True) Then
                    ...
                    FacOperacionProforma.Id = contador.ProximoValor
                    ...
                    Dim FacOperacionProformaFactura As List(Of FacOperacionProformaFactura) = DirectCast(Me._ventana.ResultadosFacturaCobro, List(Of FacOperacionProformaFactura))
                    ModificarFacOperacionesCobroxFactura(FacOperacionProformaFactura)
                    Dim exitoso As Boolean = _FacOperacionProformaServicios.InsertarOModificar(FacOperacionProforma, UsuarioLogeado.Hash)

                    If exitoso Then
                        AgregarFacFormaCobro(FacFormas, FacOperacionProforma)
                        AgregarFacOperacionProformaFacturaCobro(FacOperacionProformaFactura, FacOperacionProforma)

                        Dim operacion As New FacOperacion
                        operacion.Id = "NP"
                        operacion.CodigoOperacion = FacOperacionProforma.Id
                        operacion.FechaOperacion = FacOperacionProforma.FechaCobro
                        operacion.Asociado = FacOperacionProforma.Asociado
                        operacion.Idioma = FacOperacionProforma.Idioma
                        operacion.Moneda = FacOperacionProforma.Moneda
                        ...
                        Me.Navegar(Recursos.MensajesConElUsuario.fac_FacOperacionProformaInsertado, False)
                    End If
                    'Else
                    'Me._ventana.Mensaje(Recursos.MensajesConElUsuario.fac_ErrorFacOperacionProformaRepetido)
                    ' End If
                End If
            Catch ex As ApplicationException
                logger.[Error](ex.Message)
        ...
        Public Sub AgregarFacFormaCobro(ByVal FacFormas As List(Of FacForma), ByVal Cobro As FacOperacionProforma)
            For i As Integer = 0 To FacFormas.Count - 1
        ...
        Public Sub ModificarFacOperacionesCobroxFactura(ByVal CobroxFacturas As List(Of FacOperacionProformaFactura))
        ...
        Public Sub AgregarFacOperacionProformaFacturaCobro(ByVal FacOperacionProformaFacturas As List(Of FacOperacionProformaFactura), ByVal Cobro As FacOperacionProforma)
            For i As Integer = 0 To FacOperacionProformaFacturas.Count - 1
                FacOperacionProformaFacturas.Item(i).Id = Cobro
                Dim exitoso As Boolean = Me._FacOperacionProformaFacturaServicios.InsertarOModificar(FacOperacionProformaFacturas.Item(i), UsuarioLogeado.Hash)
            Next
        End Sub
        ...
        Public Sub AgregarFacturas2()
            ...
            Dim FacOperacionProformaFactura As New List(Of FacOperacionProformaFactura)
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    FacOperacionProformaFactura.Add(New FacOperacionProformaFactura)
                    Dim cobro As New FacOperacionProforma
                    cobro.Id = 0
                    FacOperacionProformaFactura.Item(j).Id = cobro
                    FacOperacionProformaFactura.Item(j).Factura = FacOperacion.Item(i).CodigoOperacion
                    FacOperacionProformaFactura.Item(j).Bono = FacOperacion.Item(i).Saldo
                    FacOperacionProformaFactura.Item(j).BonoBf = FacOperacion.Item(i).SaldoBf
                    If (Moneda.Id = "US") Then
                        sumabono = sumabono + FacOperacionProformaFactura.Item(j).Bono
                    Else
                        If (Moneda.Id = "BS") Then
                            sumabono = sumabono + FacOperacionProformaFactura.Item(j).Bono / 2.15
                        Else
                            If (Moneda.Id = "BF") Then
                                sumabono = sumabono + FacOperacionProformaFactura.Item(j).BonoBf
                            End If
                        End If

                    End If
                    sumabonobf = sumabonobf + FacOperacionProformaFactura.Item(j).BonoBf

                    If (xoperacion = "") Then
                        xoperacion = FacOperacionProformaFactura.Item(j).Factura
                    Else
                        xoperacion = xoperacion & "," & FacOperacionProformaFactura.Item(j).Factura
                    End If

                    j = j + 1
                End If
            Next
            Me._ventana.ResultadosFacturaCobro = FacOperacionProformaFactura
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    ...
                    If (FacOperacion.Item(i).Moneda.Id = "US") Then
                        ...
                        tasa.Id = FacOperacionProforma.FechaCobro
