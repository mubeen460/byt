...
Namespace Presentadores.FacOperacionDetaProformas
    Class PresentadorConsultarFacOperacionDetaProformas
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacOperacionDetaProformas
        ...
        Public Sub New(ByVal ventana As IConsultarFacOperacionDetaProformas)
            Try
                Me._ventana = ventana
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._ventana.Resultados = Me._FacOperacionDetaProformas
                Me._ventana.FacOperacionDetaProformaFiltrar = New FacOperacionDetaProforma()
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacOperacionDetaProformaAuxiliar As New FacOperacionDetaProforma()
                Dim FacOperacionDetaProformas As FacOperacionDetaProforma = DirectCast(_ventana.FacOperacionDetaProformaFiltrar, FacOperacionDetaProforma)

                If Not Me._ventana.Id.Equals("") Then
                    FacOperacionDetaProformaAuxiliar.Id = Integer.Parse(Me._ventana.Id)
                End If

                'If Not Me._ventana.CreditoSent.Equals("") Then
                '    FacOperacionDetaProformaAuxiliar.CreditoSent = Integer.Parse(Me._ventana.CreditoSent)
                'End If

                If (Me._ventana.Asociado IsNot Nothing) AndAlso (DirectCast(Me._ventana.Asociado, Asociado).Id <> Integer.MinValue) Then
                    FacOperacionDetaProformaAuxiliar.Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                End If

                If (Me._ventana.Banco IsNot Nothing) AndAlso (DirectCast(Me._ventana.Banco, FacBanco).Id <> Integer.MinValue) Then
                    FacOperacionDetaProformaAuxiliar.Banco = DirectCast(Me._ventana.Banco, FacBanco)
                End If

                If (Me._ventana.Idioma IsNot Nothing) AndAlso (DirectCast(Me._ventana.Idioma, Idioma).Id <> "") Then
                    FacOperacionDetaProformaAuxiliar.Idioma = DirectCast(Me._ventana.Idioma, Idioma)
                End If

                If (Me._ventana.Moneda IsNot Nothing) AndAlso (DirectCast(Me._ventana.Moneda, Moneda).Id <> "") Then
                    FacOperacionDetaProformaAuxiliar.Moneda = DirectCast(Me._ventana.Moneda, Moneda)
                End If

                If Not Me._ventana.FechaCobro.Equals("") Then
                    Dim FechaFacOperacionDetaProforma As DateTime = DateTime.Parse(Me._ventana.FechaCobro)
                    FacOperacionDetaProformaAuxiliar.FechaCobro = FechaFacOperacionDetaProforma
                End If
