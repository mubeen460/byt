...
Namespace Presentadores.FacReportes
    Class PresentadorFacturasDigitales
        Inherits PresentadorBase

        Private _ventana As IFacturasDigitales
        'Private _FacFactura As FacFactura
        Private _FacFacturaTotalZ As List(Of FacFacturaTotalZ)
        'Dim _FacFacturaDetalle As List(Of FacFactuDetalle)

        Private _FacFacturaServicios As IFacFacturaServicios
        Private _FacFacturaTotalZServicios As IFacFacturaTotalZServicios
        ...
        Public Sub New(ByVal ventana As IFacturasDigitales)
            Try
                Me._ventana = ventana
                Me._FacFacturaTotalZServicios = DirectCast(Activator.GetObject(GetType(IFacFacturaTotalZServicios), ConfigurationManager.AppSettings("RutaServidor") + ConfigurationManager.AppSettings("FacFacturaTotalZServicios")), IFacFacturaTotalZServicios)
        ...
        Public Sub Limpiar()
            ...
            Me.Navegar(New Diginsoft.Bolet.Cliente.Fac.Ventanas.FacReportes.FacturasDigitales())
        ...
        Public Sub Reporte()
            ...
            Try
                ...
                'para generar el factura totz 
                Dim facfacturatotalzaux As New FacFacturaTotalZ

                If Me._ventana.Fecha1 IsNot Nothing And Me._ventana.Fecha2 IsNot Nothing Then
                    facfacturatotalzaux.FechaDesde = Me._ventana.Fecha1
                    facfacturatotalzaux.FechaHasta = Me._ventana.Fecha2
                End If
                If Me._ventana.TipoFactura = "Factura" Then
                    'xterrero in (1,2)
                    facfacturatotalzaux.Status = 1
                End If
                If Me._ventana.TipoFactura = "Statement" Then
                    facfacturatotalzaux.Status = 2
                End If
                _FacFacturaTotalZ = _FacFacturaTotalZServicios.ObtenerFacFacturaTotalZsFiltro(facfacturatotalzaux)
                '_FacFacturaTotalZ = _FacFacturaTotalZServicios.ConsultarTodos
        ...
        Public Function ObtenerEstructuraEnc(ByVal total_z As FacFacturaTotalZ) As StructReporteFActuraEnc
        ...
        Private Sub ObtenerEstructura(ByRef enc As IList(Of StructReporteFActuraEnc), ByRef det As List(Of StructReporteFActuraDeta))
            ...
            Dim j As Integer = 0
            Try
                For i As Integer = 0 To _FacFacturaTotalZ.Count - 1
                    If i = 0 Then
                        'llamar encabezado
                        structura = ObtenerEstructuraEnc(_FacFacturaTotalZ(i))
                        j = j + 1
                        structura.Id = j
                        retorno.Add(structura)
                        ObtenerEstructuraDeta(j, det, _FacFacturaTotalZ(i).Id)
                    Else
                        If _FacFacturaTotalZ(i).Id <> _FacFacturaTotalZ(i - 1).Id Then
                            'llamar encabezado
                            structura = ObtenerEstructuraEnc(_FacFacturaTotalZ(i))
                            j = j + 1
                            structura.Id = j
                    ...
                    If i = _FacFacturaTotalZ.Count - 1 Then
                        'llamar detalle
                    Else
                        If _FacFacturaTotalZ(i).Id <> _FacFacturaTotalZ(i + 1).Id Then
                            'llamar detalle
