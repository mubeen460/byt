...
Namespace Presentadores.FacOperacionDetalles
    Class PresentadorAgregarFacOperacionDetalle
        Inherits PresentadorBase
        Private _ventana As IAgregarFacOperacionDetalle
        ...
        Private _FacOperacionDetalleFacturaServicios As IFacOperacionDetalleFacturaServicios
        Dim xoperacion As String
        ''' <summary>
        ''' Constructor predeterminado
        ''' </summary>
        ''' <param name="ventana">Página que satisface el contrato</param>
        Public Sub New(ByVal ventana As IAgregarFacOperacionDetalle)
            Try
                Me._ventana = ventana
                'Me._ventana.FacOperacionDetalle = New FacOperacionDetalle()
                ...
                Me._FacOperacionDetalleFacturaServicios = DirectCast(Activator.GetObject(GetType(IFacOperacionDetalleFacturaServicios), ConfigurationManager.AppSettings("RutaServidor") + ConfigurationManager.AppSettings("FacOperacionDetalleFacturaServicios")), IFacOperacionDetalleFacturaServicios)

                Dim FacOperacionDetalle As New FacOperacionDetalle()
        ...
        Public Sub CargarPagina()
            ...
            Try
                Me.ActualizarTituloVentanaPrincipal(Recursos.Etiquetas.fac_titleAgregarFacOperacionDetalle, Recursos.Ids.AgregarUsuario)
                Me._ventana.MensajeErrorCobro = ""

                Dim FacOperacionDetalle As New FacOperacionDetalle()
                FacOperacionDetalle.FechaCobro = Date.Now
                Me._ventana.FacOperacionDetalle = FacOperacionDetalle
        ...
        Public Sub Aceptar()
            Try
                Dim FacOperacionDetalle As FacOperacionDetalle = DirectCast(_ventana.FacOperacionDetalle, FacOperacionDetalle)

                FacOperacionDetalle.Asociado = If(Not DirectCast(Me._ventana.Asociado, Asociado).Id.Equals("NGN"), DirectCast(Me._ventana.Asociado, Asociado), Nothing)

                'If Not Me._FacCreditoServicios.VerificarExistencia(FacCredito) Then                
                FacOperacionDetalle.Moneda = If(Not DirectCast(Me._ventana.Moneda, Moneda).Id.Equals("NGN"), DirectCast(Me._ventana.Moneda, Moneda), Nothing)
                FacOperacionDetalle.Idioma = If(Not DirectCast(Me._ventana.Idioma, Idioma).Id.Equals("NGN"), DirectCast(Me._ventana.Idioma, Idioma), Nothing)
                FacOperacionDetalle.Banco = If(Not DirectCast(Me._ventana.Banco, FacBanco).Id.Equals("NGN"), DirectCast(Me._ventana.Banco, FacBanco), Nothing)

                Dim valido As Boolean = True
                If (FacOperacionDetalle.Moneda.Id = "US") Then
                ...
                If (valido = True) Then
                    ...
                    FacOperacionDetalle.Id = contador.ProximoValor
                    ...
                    Dim FacOperacionDetalleFactura As List(Of FacOperacionDetalleFactura) = DirectCast(Me._ventana.ResultadosFacturaCobro, List(Of FacOperacionDetalleFactura))
                    ModificarFacOperacionesCobroxFactura(FacOperacionDetalleFactura)
                    Dim exitoso As Boolean = _FacOperacionDetalleServicios.InsertarOModificar(FacOperacionDetalle, UsuarioLogeado.Hash)

                    If exitoso Then
                        AgregarFacFormaCobro(FacFormas, FacOperacionDetalle)
                        AgregarFacOperacionDetalleFacturaCobro(FacOperacionDetalleFactura, FacOperacionDetalle)

                        Dim operacion As New FacOperacion
                        operacion.Id = "NP"
                        operacion.CodigoOperacion = FacOperacionDetalle.Id
                        operacion.FechaOperacion = FacOperacionDetalle.FechaCobro
                        operacion.Asociado = FacOperacionDetalle.Asociado
                        operacion.Idioma = FacOperacionDetalle.Idioma
                        operacion.Moneda = FacOperacionDetalle.Moneda
                        ...
                        Me.Navegar(Recursos.MensajesConElUsuario.fac_FacOperacionDetalleInsertado, False)
                    End If
                    'Else
                    'Me._ventana.Mensaje(Recursos.MensajesConElUsuario.fac_ErrorFacOperacionDetalleRepetido)
                    ' End If
                End If
            Catch ex As ApplicationException
                logger.[Error](ex.Message)
        ...
        Public Sub AgregarFacFormaCobro(ByVal FacFormas As List(Of FacForma), ByVal Cobro As FacOperacionDetalle)
            For i As Integer = 0 To FacFormas.Count - 1
        ...
        Public Sub ModificarFacOperacionesCobroxFactura(ByVal CobroxFacturas As List(Of FacOperacionDetalleFactura))
        ...
        Public Sub AgregarFacOperacionDetalleFacturaCobro(ByVal FacOperacionDetalleFacturas As List(Of FacOperacionDetalleFactura), ByVal Cobro As FacOperacionDetalle)
            For i As Integer = 0 To FacOperacionDetalleFacturas.Count - 1
                FacOperacionDetalleFacturas.Item(i).Id = Cobro
                Dim exitoso As Boolean = Me._FacOperacionDetalleFacturaServicios.InsertarOModificar(FacOperacionDetalleFacturas.Item(i), UsuarioLogeado.Hash)
            Next
        End Sub
        ...
        Public Sub AgregarFacturas2()
            ...
            Dim FacOperacionDetalleFactura As New List(Of FacOperacionDetalleFactura)
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    FacOperacionDetalleFactura.Add(New FacOperacionDetalleFactura)
                    Dim cobro As New FacOperacionDetalle
                    cobro.Id = 0
                    FacOperacionDetalleFactura.Item(j).Id = cobro
                    FacOperacionDetalleFactura.Item(j).Factura = FacOperacion.Item(i).CodigoOperacion
                    FacOperacionDetalleFactura.Item(j).Bono = FacOperacion.Item(i).Saldo
                    FacOperacionDetalleFactura.Item(j).BonoBf = FacOperacion.Item(i).SaldoBf
                    If (Moneda.Id = "US") Then
                        sumabono = sumabono + FacOperacionDetalleFactura.Item(j).Bono
                    Else
                        If (Moneda.Id = "BS") Then
                            sumabono = sumabono + FacOperacionDetalleFactura.Item(j).Bono / 2.15
                        Else
                            If (Moneda.Id = "BF") Then
                                sumabono = sumabono + FacOperacionDetalleFactura.Item(j).BonoBf
                            End If
                        End If

                    End If
                    sumabonobf = sumabonobf + FacOperacionDetalleFactura.Item(j).BonoBf

                    If (xoperacion = "") Then
                        xoperacion = FacOperacionDetalleFactura.Item(j).Factura
                    Else
                        xoperacion = xoperacion & "," & FacOperacionDetalleFactura.Item(j).Factura
                    End If

                    j = j + 1
                End If
            Next
            Me._ventana.ResultadosFacturaCobro = FacOperacionDetalleFactura
            ...
            For i As Integer = 0 To FacOperacion.Count - 1
                If (FacOperacion.Item(i).Seleccion = True) Then
                    ...
                    If (FacOperacion.Item(i).Moneda.Id = "US") Then
                        ...
                        tasa.Id = FacOperacionDetalle.FechaCobro
