...
Namespace Presentadores.FacOperacionDetaAnuladas
    Class PresentadorConsultarFacOperacionDetaAnuladas
        Inherits PresentadorBase
        Private Shared _paginaPrincipal As PaginaPrincipal = PaginaPrincipal.ObtenerInstancia
        Private Shared logger As Logger = LogManager.GetCurrentClassLogger()

        Private _ventana As IConsultarFacOperacionDetaAnuladas
        ...
        Public Sub New(ByVal ventana As IConsultarFacOperacionDetaAnuladas)
            Try
                Me._ventana = ventana
        ...
        Public Sub CargarPagina()
            ...
            Try
                ...
                Me._ventana.Resultados = Me._FacOperacionDetaAnuladas
                Me._ventana.FacOperacionDetaAnuladaFiltrar = New FacOperacionDetaAnulada()
        ...
        Public Sub Consultar()
            Try
                ...
                Dim FacOperacionDetaAnuladaAuxiliar As New FacOperacionDetaAnulada()
                Dim FacOperacionDetaAnuladas As FacOperacionDetaAnulada = DirectCast(_ventana.FacOperacionDetaAnuladaFiltrar, FacOperacionDetaAnulada)

                If Not Me._ventana.Id.Equals("") Then
                    FacOperacionDetaAnuladaAuxiliar.Id = Integer.Parse(Me._ventana.Id)
                End If

                'If Not Me._ventana.CreditoSent.Equals("") Then
                '    FacOperacionDetaAnuladaAuxiliar.CreditoSent = Integer.Parse(Me._ventana.CreditoSent)
                'End If

                If (Me._ventana.Asociado IsNot Nothing) AndAlso (DirectCast(Me._ventana.Asociado, Asociado).Id <> Integer.MinValue) Then
                    FacOperacionDetaAnuladaAuxiliar.Asociado = DirectCast(Me._ventana.Asociado, Asociado)
                End If

                If (Me._ventana.Banco IsNot Nothing) AndAlso (DirectCast(Me._ventana.Banco, FacBanco).Id <> Integer.MinValue) Then
                    FacOperacionDetaAnuladaAuxiliar.Banco = DirectCast(Me._ventana.Banco, FacBanco)
                End If

                If (Me._ventana.Idioma IsNot Nothing) AndAlso (DirectCast(Me._ventana.Idioma, Idioma).Id <> "") Then
                    FacOperacionDetaAnuladaAuxiliar.Idioma = DirectCast(Me._ventana.Idioma, Idioma)
                End If

                If (Me._ventana.Moneda IsNot Nothing) AndAlso (DirectCast(Me._ventana.Moneda, Moneda).Id <> "") Then
                    FacOperacionDetaAnuladaAuxiliar.Moneda = DirectCast(Me._ventana.Moneda, Moneda)
                End If

                If Not Me._ventana.FechaCobro.Equals("") Then
                    Dim FechaFacOperacionDetaAnulada As DateTime = DateTime.Parse(Me._ventana.FechaCobro)
                    FacOperacionDetaAnuladaAuxiliar.FechaCobro = FechaFacOperacionDetaAnulada
                End If
