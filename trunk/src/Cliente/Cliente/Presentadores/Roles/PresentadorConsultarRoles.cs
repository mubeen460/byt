...
namespace Trascend.Bolet.Cliente.Presentadores.Roles
{
    class PresentadorConsultarRoles : PresentadorBase
    {
        private IConsultarRoles _ventana;
        ...
        public PresentadorConsultarRoles(IConsultarRoles ventana)
        {
            try
            {
        ...
        public void Consultar()
        {
            try
            {
                ...
                IEnumerable<Rol> rolesFiltrados = this._roles;

                if (!string.IsNullOrEmpty(this._ventana.Id))
                {
                    rolesFiltrados = from rol in rolesFiltrados
                                     where rol.Id.Contains(this._ventana.Id)
                                     select rol;
                }
                ...
                {
                    rolesFiltrados = from rol in rolesFiltrados
                                     where rol.Descripcion != null && 
                                     rol.Descripcion.Contains(this._ventana.Descripcion)
                                     select rol;
                }
        ...
        /// <summary>
        /// Método que invoca una nueva página "ConsultarRol" y la instancia con el rol seleccionado
        /// </summary>
        public void IrConsultarRol()
        {
            #region trace
            if (ConfigurationManager.AppSettings["ambiente"].ToString().Equals("desarrollo"))
                logger.Debug("Entrando al metodo {0}", (new System.Diagnostics.StackFrame()).GetMethod().Name);
            ...
            if (this._ventana.RolSeleccionado != null)
                this.Navegar(new ConsultarRol(this._ventana.RolSeleccionado));
