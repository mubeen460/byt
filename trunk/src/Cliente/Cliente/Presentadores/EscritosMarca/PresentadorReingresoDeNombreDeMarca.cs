...
namespace Trascend.Bolet.Cliente.Presentadores.EscritosMarca
{
    class PresentadorReingresoDeNombreDeMarca : PresentadorBase
    {
            ...
            try
            {
                ...
                if (ValidarEscrito())
                {
                    ...
                    this.EjecutarArchivoBAT(ConfigurationManager.AppSettings["RutaBatEscrito"].ToString()
                        ...
                        + " " + ((ListaDatosValores)this._ventana.CantidadNumeralSelec).Valor + " " + this._ventana.Numerales
                        + " " + this._ventana.CalificativoNombreMarca + " " + ((Agente)this._ventana.AgenteFiltrado).Id 
                        + " " + parametroMarcas);                  
        ...
        /// <summary>
        /// Método que se encarga de validar antes de generar el escrito
        /// </summary>
                    ...
                    if (this.ValidarAgenteApoderadoDeMarcas((Agente)this._ventana.AgenteFiltrado, this._marcasAgregadas))
                    {
                        if (((Boletin)this._ventana.Boletin != null) && (((Boletin)this._ventana.Boletin).Id != int.MinValue))
                        {
                            if (this._ventana.Resolucion != null)
                            {
                                ...
                                {
                                    if (!this._ventana.CalificativoNombreMarca.Equals(""))
                                    {
                                        retorno = true;
                                    }
                                    else 
                                    {
                                        this._ventana.MensajeAlerta(string.Format(Recursos.MensajesConElUsuario.AlertaEscritoSinCalificativoDeMarca));
        ...
        #region Boletines Numerales y Resoluciones
        ...
        /// <summary>
        /// Método que hace la carga de numerales
        /// </summary>
